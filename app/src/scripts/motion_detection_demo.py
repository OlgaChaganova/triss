...
class MotionDetectorAdaptative():
    ...
    def initRecorder(self):  # Create the recorder
        ...
        self.writer = cv.VideoWriter(datetime.now().strftime("%b-%d_%H_%M_%S") + ".wmv", codec, 5, self.frame.shape[1::-1],
                                     1)
        ...
        while True:
            ...
            if not self.isRecording:
                ...
                if instant >= self.trigger_time + 10:  # Record during 10 seconds
                    ...
                    cv.putText(currentframe, datetime.now().strftime("%b %d, %H:%M:%S"), (25, 30), self.font, 1,
                             (255, 0, 0), 2, cv.LINE_AA)  # Put date on the frame
    ...
    def processImage(self, curframe):
        ...
        self.gray_frame = cv.dilate(self.gray_frame, self.es)


    def somethingHasMoved(self):
        # Find contours, ignore other return values (image, contours, hierarchy)[1]
        contours = cv.findContours(self.gray_frame, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)[0]
